...
using namespace std;
class A{
public: int a; A(int x):a(x){cout<<"(A::construct1) "<<endl;}
...
class B:public A{
public: int a; B(int x):a(x){cout<<"(B::construct) "<<endl;}
...
//(B::construct) 
////测试调用重载函数
//(B::f) my a is:12 and my base's a is:99
////测试基类函数语句里调用子类重载函数
//(A::trouble) (B::f) my a is:12 and my base's a is:99
////测试调用const的重载函数
//(B::f const) my a is:12 and my base's a is:99
////测试调用非重载函数
//(A::construct0) 
//(B::construct) 
//(A::t const) my a is:99
