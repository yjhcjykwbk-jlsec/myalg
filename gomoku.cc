...
enum ChessType{Empty=0,White=1,Black=2};
struct Pos{
	int x,y;
...
struct Chess{
	...
	int getType(Pos &p){
		if(p.x<0||p.x>=xnum) return -1;
		if(p.y<0||p.y>=ynum) return -1;
...
bool nextWin(ChessType type){
	assert(type==White||type==Black);
	int num=0;bool lastIsEmpty=false;
	Pos p;int t;int i,j;
	//枚举横向(0度角)
	rep0(j,chess.ynum){
		p.set(0,j);
		while((t=chess.getType(p))>=0){
			if(t==3-type) num=0;
			else if(t==type||!lastIsEmpty){
				num++;
				if(num>=5) return true;
				if(t==Empty) lastIsEmpty=true;
			}
			else //lastIsEmpty 
				num=1;
			p.change(1,0);
...
int main(){
	...
	while(scanf("%d",&n)){
		...
		rep0(i,n){
			scanf("%d %d %d\n",&x,&y,&type);
			chess.set(x,y,(ChessType)type);
		}
		if(numOf(Black)<numOf(White)){
			printf("Invalid\n");
			return 0;
		}
		ChessType type=whoIsNext();
		if(nextWin(type)) printf("next will win\n");
	}
