...
int *a,int *b;//a长度比b短
int len1,len2,midlen;
int med(int l1,int r1,int l2,int r2){
	if(l1==r1||l2==r2){
		if(a[l1]>b[l2]) return a[l1];
		return b[l2];
	}
	int mid1=(l1+r1)/2;//包含l1
	int mid2=midlen-(mid1+1);//
	if(mid2<l2||mid2>r2){
		return a[mid1];
	}
	//缩小选择范围
	if(a[mid1]<b[mid2]) return med(mid1+1,r1,midlen-(r1+1+1),mid2);
	if(a[mid1]>b[mid2]) return med(l1,mid1,mid2+1,r2);
	return a[mid1];
}
int main(){
	scanf("%d%d",len1,len2);
	midlen=(len1+len2-1)/2;//前方有midlen个元素
