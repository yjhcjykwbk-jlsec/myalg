...
void init(){
  ...
  for(int i=1;i<N;i++){
	int a,b;cin>>a>>b;a--,b--; E[a][b]=E[b][a]=true;
  }
  queue<int> q; bool vis[MAXN]; memset(vis,0,sizeof(vis)); q.push(0);vis[0]=true;
  while(!q.empty()){
	int v=q.front();q.pop();
	for(int i=0;i<N;i++) 
	  if(E[v][i]&&vis[i]==false){Edges[v].push_back(i);vis[i]=true;q.push(i);}
  }
  for(int i=0;i<N;i++) tree[i][0]=tree[i][1]=tree[i][2]=MAX;
}
bool dfs(int v){
  ...
  for(int i=0;i<Edges[v].size();i++){
	...
	int s2=tree[s][2];
	if(num<MAX&&s2<MAX){
	  if(tree[v][0]>s2+num+1) tree[v][0]=s2+num+1;
	}
	for(int j=i+1;j<Edges[v].size();j++){
	  ...
	  if(s12<MAX&&t12<MAX){
		if(tree[v][0]>s12+t12+num+1) tree[v][0]=s12+t12+num+1;
	  }
	}
  }
