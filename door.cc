...
int posy[MAXN];
int n;
int num[MAXM];//记录每一墙的下标
int atx[MAXM];
bool touchWall(
		return false;
		}
double getDst(int i,int j){//i<j 计算两个点之间的直线距离，如果有墙体遮挡，则返回错误
	double px=posx[i],py=posy[i];
	double 	dy=posy[j]-posy[i];
	double 	dx=posx[j]-posx[i];
	if(!touchWall(px,py,posx[j]))
		return  dst(px,py,posx[j],posy[j]);
	else return FAR;
}
void calc(){
	rep(i,n)
		rep( j,num[i],num[i+1]){//MAXN
			Dst[j]=FAR;
			rep(k,1,num[i]){
				int tmp;
				if((tmp=dst[k]+getDst(k,j))<Dst[j]) Dst[j]=tmp;
			}
		}
}
struct wall{
	float l,r;
	bool operator <(const wall &t){
		return l<t.l;
	}
	wall(int a,int b){l=a;r=b;}
};
vector<wall> walls[MAXM];
int M,N;
int main(){
	cin>>M;//
	rep(i,MAXM){
		float a,b;cin>>a>>b;
		walls[i].push(wall(a,b));
	}
}
